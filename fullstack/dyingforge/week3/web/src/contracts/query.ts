...
// Add interface for the GraphQL response type
...
export const queryFolderDataByGraphQL = async (folderAddress: string): Promise<FolderData[]> => {
    ...
    try {
        ...
        }) as FolderDataResponse;  // Add type assertion here

        // 转换查询结果为 FolderData[]
        const dynamicFields = response.data?.object.dynamicFields || [];
       const folderdata:FolderData[] =  dynamicFields.map((field) => ({
...
// public fun add_nft_to_folder<T: store + key>(folder: &mut Folder, nft: T, ctx: &mut TxContext) {
//     let type_name = type_name::get<T>();
//     let nft_name = object::id_to_address(&object::id(&nft));
//     dynamic_object_field::add(&mut folder.id, nft_name, nft);
...
//     })
// }
export const addNftToFolderTx = async (nft: string, folder: string ,nft_type: string) => {
    ...
    tx.moveCall({
        ...
        arguments: [tx.object(folder), tx.object(nft)],
