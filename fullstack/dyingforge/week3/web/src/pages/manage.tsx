...
import { addCoinToFolderTx, createFolderTx, queryFolderDataByGraphQL, addNftToFolderTx} from "@/contracts/query";
...
const User = () => {
    ...
    const handleAddNftToFolder = async (asset: SuiObject) => {
        if (!selectedFolder) {
            console.log("No folder selected");
            return;
        }
        const tx = await addNftToFolderTx(
            asset.id,
                                                ...
                                                <div key={index} className="p-6 bg-white border border-gray-100 rounded-lg shadow-sm hover:shadow-md transition-shadow">
                                                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                                                        <div className="flex flex-col items-start gap-2 min-w-0 flex-1">
                                                            ...
                                                            {!isNft && asset.hasOwnProperty('balance') && (
                                                        ...
                                                        <div className="flex items-center gap-3 shrink-0">
                                                            ...
                                                            <Button 
                                                                onClick={() => isNft ? handleAddNftToFolder(asset) : handleAddToFolder(asset)}
